...
// templates). We need to map these wrapper rules to match our base config.

// Import base config to get common rules
...
// Extract rules from base config
const baseRules = baseConfig.find( ( config ) => config.rules && Object.keys( config.rules ).length > 50 )?.rules || {};

const wrappedRules = {};
for ( const rule of rulesToMap ) {
	if ( rule in baseRules ) {
		wrappedRules[ 'vue/' + rule ] = baseRules[ rule ];
	}
}
